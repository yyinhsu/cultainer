...
### AI 助手（Gemini）
- 在**個人頁 > 設定 > Gemini API Key** 設定 API 金鑰（含驗證功能，裝置端加密儲存）
- 一鍵分析摘錄內容
- 彙整多筆摘錄為統一摘要
- AI 潤飾心得評論
- 分析結果可儲存於摘錄中供日後參考
...
### 探索與推薦
- 基於創作者的推薦：發現你喜歡的導演、作者、藝人的其他作品
- 類型篩選（全部、書籍、電影、影集、音樂）
- 推薦卡片依創作者分組，附帶推薦來源（「因為你記錄了…」）
- 從推薦詳情快速加入願望清單
- 離線支援：推薦結果快取，1 小時 TTL

### 月曆
- 月曆視圖，有紀錄的日期以圓點標示
- 月份導航（上/下月）及「Today」快速跳轉
- 點選日期查看該日所有紀錄
- 從首頁標題列的日曆圖示進入
...
### 平台與桌面支援
- 響應式佈局：桌面版使用側邊欄導航，行動版使用底部 Tab Bar
- macOS 原生選單列，支援鍵盤快捷鍵（Cmd+1-4 頁面切換，Cmd+N 新增紀錄）
- 桌面版互動卡片支援滑鼠 hover 效果
- PWA 支援，Web 版具備離線功能
- 隱私政策與使用條款可在 App 內查看

### CI/CD
- GitHub Actions 流程：每次推送至 main 時自動分析、測試，並建置 Web、iOS、Android 版本
